...
namespace GameOfLifeState
{
    public class GameOfLifeGame
    {
        ...
        private Cell[,] UpdateBoard(Cell[,] board) // update board
        {
            var newboard = new Cell[MaxRow, MaxColumn];
            for (var i = 0; i <= 11; i++)
            {
                for (var j = 0; j <= 11; j++)
                {
                    ...
                    if (neighbors < 2)
                    {
                        newboard[i, j] = CreateDeadCell(i, j); // underpopulation, dies
                    }

                    if (neighbors > 3)
                    {
                        newboard[i, j] = CreateDeadCell(i, j); // overpopulation, dies
                    }

                    if (!board[i, j].IsAlive() && neighbors == 3)
                    {
                        newboard[i, j] = CreateAliveCell(i, j); // a cell is born!
                    }

                    if (neighbors >= 2 && neighbors <= 3 && board[i, j].IsAlive())
                    {
                        newboard[i, j] = CreateAliveCell(i, j); // else same as given
                    }

                    if (newboard[i, j] == null)
                    {
                        newboard[i, j] = CreateDeadCell(i, j);
                    }
                }
            }
        ...
        {
            return new Cell(row, column) { State = new DeadState() };
        }

        private static Cell CreateAliveCell(int row, int column)
        {
            return new Cell(row, column) { State = new AliveState() };
        }
    }
}
